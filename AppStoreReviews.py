...
def _getReviewsForPage(appStoreId, appId, pageNo):
    userAgent = 'iTunes/10.3.1 (Macintosh; Intel Mac OS X 10.6.8) AppleWebKit/533.21.1'
...
def _print_reviews(reviews, country):
    ...
    if len(reviews)>0:
        ...
        for review in reviews:
            print "%s by %s" % (unicodedata.normalize('NFKD', unicode(review["version"])).encode('ascii','ignore'), unicodedata.normalize('NFKD', unicode(review["user"])).encode('ascii','ignore'))
            print " (%s) %s" % (unicodedata.normalize('NFKD', unicode(review["topic"])).encode('ascii','ignore'), unicodedata.normalize('NFKD',unicode(review["review"])).encode('ascii','ignore'))
            print ""
            sumRank += review["rank"]
